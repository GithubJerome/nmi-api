...
"""Tutor Results Overview"""
from flask import  request
from library.common import Common
from library.postgresql_queries import PostgreSQL

class TutorResultsOverview(Common):
    """Class for TutorResultsOverview"""

    # INITIALIZE
    def __init__(self):
        """The Constructor for TutorResultsOverview class"""
        self.postgresql_query = PostgreSQL()
        super(TutorResultsOverview, self).__init__()

    def tutor_results_overview(self):
        """
        This API is for Getting Tutor Results Overview
        ---
        tags:
          - Tutor Progress
        produces:
        ...
        data['data'] = self.get_tutor_course(userid)
        data['status'] = 'ok'

        return self.return_data(data, userid)

    def get_tutor_course(self, user_id):
        """Return All Courses"""

        # DATA
        sql_str = "SELECT * FROM tutor_courses sc"
        ...
        not_in = [None, 0]
        progress = [float(res['progress']) for res in results if res['progress'] not in not_in]
    ...
    def get_course_activity(self, user_id, course_id):
        ...
        sql_str = "SELECT s.section_name, sub.subsection_name, se.* FROM tutor_exercise se"
        ...
        if result:
            for res in result:
                ...
                exercise = self.get_activity_details(res['tutor_exercise_id'])
                ...
                self.remove_key(res, "tutor_exercise_id")
    ...
    def get_activity_details(self, tutor_exercise_id):
